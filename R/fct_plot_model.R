...
plot_prediction <- function(prediction_df, log = TRUE, lang = 'en') {
  ...
  if(lang == 'en') {
    p <- p +
      ...
      ggplot2::labs(title = "Prediction model results", x = "Week", y = "Lice per Fish") +
      ggplot2::scale_x_discrete(labels = c(1, 2, 1, 2, 1, 2)) +
      ggplot2::scale_color_discrete(name = "", labels = c("Sessile", "Motile", "Female"))
  }

  if(lang == 'nb'){
    p <- p +
      ggplot2::facet_wrap( ~ cage_name_nb, ncol = 1) +
      ggplot2::labs(title = "Prediksjonsmodellresultater", x = "Uke", y = "Lus per fisk") +
      ggplot2::scale_x_discrete(labels = c(1, 2, 1, 2, 1, 2)) +
      ggplot2::scale_color_discrete(name = "", labels =  c("Fastsittende", "Mobile", "Hunnlus"))
  }
