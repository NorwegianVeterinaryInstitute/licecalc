...
mod_lice_model_v2_ui <- function(id) {
  ...
  tagList(shiny::sidebarLayout(
    shiny::sidebarPanel(
      ...
      shiny::radioButtons(
        ...
        choices = setNames(
          ...
          c(
            ...
            "Select infection pressure and sea temperature manually"
          )
        ),
        selected = 2
      ),
      ...
      shiny::conditionalPanel(
        condition = sprintf("input[['%s']] === '2'", ns("infectious_pressure")),
        div(
          class = "d-flex flex-column",
          shiny::sliderInput(
            ns("infectious_pressure_manually"),
            label = i18n$t("Infectious presure"),
            min = 0,
            max = 22,
            step = 1,
            ticks = FALSE,
      ...
      shiny::mainPanel(DT::dataTableOutput(ns(
        "manual_data_table"
      )),
      shiny::plotOutput(ns(
        "prediction_plot"
...
mod_lice_model_v2_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      choices_radio_1 <- setNames(1:2, i18n_r()$t(
        c(
          "Enter the 5-digit locality number and get infection pressure automatically",
    ...
    output$prediction_plot <- shiny::renderPlot({
      ...
      } else if (input$additional_data == 1) {
        tryCatch(
          expr = {
            make_plot_for_cages_and_location(location = input$locality_number,
                                             user_data = user_data())
